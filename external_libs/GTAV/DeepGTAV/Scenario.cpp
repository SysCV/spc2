...
void Scenario::parseDatasetConfig(const Value& dc, bool setDefaults, LaneRewarder *lanerewarder) {
...
void Scenario::buildScenario() {
	...
	if (_drivingMode >= 0) {
		...
		else AI::TASK_VEHICLE_DRIVE_TO_COORD(ped, vehicle, dir.x, dir.y, dir.z, _setSpeed, p6, vehicleHash, _drivingMode, 1.0, true);
	}
	UI::REFRESH_WAYPOINT();
	if (direction) UI::SET_NEW_WAYPOINT(dir.x, dir.y);
}

void Scenario::start(const Value& sc, const Value& dc, LaneRewarder* lanerewarder) {
	if (is_slow_motion)
	...
	parseDatasetConfig(dc, true, lanerewarder);
...
void Scenario::config(const Value& sc, const Value& dc, LaneRewarder* lanerewarder) {
	if (!running) return;
	...
	parseDatasetConfig(dc, false, lanerewarder);
...
void Scenario::setroadinfo()
{
	...
	roadinfo.PushBack((*(rewarder->laneRewarder)).isOnroad, allocator);
	roadinfo.PushBack((*(rewarder->laneRewarder)).cannotaccessroadinfo, allocator);
	...
	if ((*(rewarder->laneRewarder)).isOnroad && (!(*(rewarder->laneRewarder)).cannotaccessroadinfo)) {
		Vector3 roadVector = (*(rewarder->laneRewarder)).road_.direction;
		roadinfo.PushBack(roadVector.x, allocator).PushBack(roadVector.y, allocator).PushBack(roadVector.z, allocator);
		roadinfo.PushBack((*(rewarder->laneRewarder)).pointPair.at(0).coord.x, allocator).PushBack((*(rewarder->laneRewarder)).pointPair.at(0).coord.y, allocator)
			.PushBack((*(rewarder->laneRewarder)).pointPair.at(0).coord.z, allocator);
		roadinfo.PushBack((*(rewarder->laneRewarder)).pointPair.at(1).coord.x, allocator).PushBack((*(rewarder->laneRewarder)).pointPair.at(1).coord.y, allocator)
			.PushBack((*(rewarder->laneRewarder)).pointPair.at(1).coord.z, allocator);
		bool laneIn = (*(rewarder->laneRewarder)).pointPair.at(0).laneIn || (*(rewarder->laneRewarder)).pointPair.at(1).laneIn;
		roadinfo.PushBack(laneIn, allocator);
		roadinfo.PushBack((*(rewarder->laneRewarder)).road.attr.cantGoLeft, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.cantGoRight, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.density, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.disabled, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.highway, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.indicateKeepLeft, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.indicateKeepRight, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.leftTurnsOnly, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.noBigVehicles, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.noGPS, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.offRoad, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.slipLane, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.special, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.speed, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.tunnel, allocator)
			.PushBack((*(rewarder->laneRewarder)).road.attr.water, allocator);
		roadinfo.PushBack((*(rewarder->laneRewarder)).road_.attr.blockIfNoLanes, allocator)
			.PushBack((*(rewarder->laneRewarder)).road_.attr.gpsBothWays, allocator)
			.PushBack((*(rewarder->laneRewarder)).road_.attr.narrowRoad, allocator)
			.PushBack((*(rewarder->laneRewarder)).road_.attr.shortcut, allocator)
			.PushBack((*(rewarder->laneRewarder)).road_.attr.width, allocator);
	}
	d["roadinfo"] = roadinfo;
}
