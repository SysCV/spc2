...
with open(os.path.join(dir_path, "sampled_path_points.np"), "rb") as f:
    x_list, y_list = np.load(f)


class GtaEnv():

    def __init__(self, seed=False, duration=1000, vehicle='oracle', weather='CLEAR', time=[10, 30], location=[-1440, -600], destination = [-1,-1,-1], autodrive = None):
        """
    ...
    def reset(self):
        ...
        time.sleep(10)  # IMPORTANT! Wait for the scene to be ready,
