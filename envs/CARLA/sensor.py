...
######### This file only works for build environment on CARLA 0.9.x ###############
...
def seg_to_bbox(seg, vehicle_id=10):
    ...
    for vehicle in measure.regionprops(vehicle_seg):
        if vehicle.area < 10:
            continue
        minr, minc, maxr, maxc = vehicle.bbox
        vehicle_bboxes.append([minr, minc, maxr, maxc])
...
class CameraManager(object):
    def __init__(self, parent_actor, args, hud=None, x=1, y=0, z=2.5, monitor=False, tag=None):
        ...
        self.bboxes = [] # for segmentation camera to store coarse bounding box list
    ...
    def set_sensor(self, index):
        ...
        if needs_respawn:
            ...
            if index == 0 and not self.monitor:
                ...
                self.output = np.zeros([self.args.frame_height, self.args.frame_width])
        ...
        self._index = index
        K = np.identity(3)
        K[0, 2] = self.args.frame_width /2.0
        K[1, 2] = self.args.frame_height / 2.0
        K[0, 0] = K[1, 1] = self.args.frame_width / (2.0 * math.tan(VIEW_FOV * math.pi / 360.0))
        self.sensor.calibration = K
